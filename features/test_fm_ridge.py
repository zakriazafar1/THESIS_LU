...
RIDGE-FEATURES -- BELANGRIJKE UPDATE:
  De ridge-frequenties (ridge_onset_hz/peak_hz/end_hz) worden nu bepaald op
  1/f-GECORRIGEERDE ABSOLUTE power, niet meer op ruwe/L2-genormaliseerde
  power. Reden: EEG-power volgt van nature ongeveer een 1/f-verdeling (lage
  frequenties hebben van nature veel meer ruwe power dan bv. sigma/beta),
  waardoor een kale argmax over ongenormaliseerde (of alleen L2-
  genormaliseerde) power bijna altijd de laagste gescande frequentie
  (RIDGE_FREQ_MIN) koos, ongeacht wat er inhoudelijk in het event gebeurde.

  De fix: er wordt éénmaal per nacht een gladde 1/f-trend gefit door de
  whole-night mediane power per frequentie (fit_1f_whitening_curve, via
  log-log lineaire regressie), en die gefitte trend wordt gebruikt om de
  event-power per frequentie te whitenen vóór de argmax. Dit corrigeert
  ALLEEN de systematische 1/f-scheefheid, niet de werkelijke nacht-
  specifieke spectrale vorm (in tegenstelling tot delen door de ruwe
  empirische baseline zelf, wat de ridge relatief t.o.v. "normaal voor
  deze nacht" zou maken). De ridge kiest dus nog steeds op basis van
  (gecorrigeerde) ABSOLUTE power -- ridge_onset_hz blijft interpreteerbaar
  als "welke frequentie domineerde", niet "welke frequentie week het meest
  af van normaal".
...
def compute_night_ridge_baseline(sig: np.ndarray, sf: float, freqs: np.ndarray,
                                  n_cycles=None, L2normalize: bool = True) -> np.ndarray:
    """
    Whole-night mediane Morlet-power PER FREQUENTIE. Dit is de ruwe,
    empirische spectrale vorm van het signaal over de hele nacht -- ruis en
    al (bv. als iemand van nature wat meer sigma-activiteit heeft, zit dat
    hier ook in). Wordt NIET direct gebruikt om per-event power tegen te
    delen (dat zou de ridge relatief maken t.o.v. "normaal voor deze
    nacht", en dus echte nacht-specifieke spectrale verschillen wegpoetsen)
    -- zie fit_1f_whitening_curve() voor de vervolgstap die hier alleen de
    systematische 1/f-trend uit haalt.

    Wordt freq-voor-freq berekend i.p.v. via compute_morlet_tf in één keer
    (die een (n_freqs, n_samples)-array zou opbouwen -- voor een hele nacht
    op 128Hz is dat al gauw >1GB voor 40 frequenties). Hier wordt per
    frequentie direct na de power-berekening de mediaan genomen en de
    rest weggegooid, dus blijft het geheugengebruik beperkt tot één
    frequentie tegelijk.
    """
    freqs = np.asarray(freqs)
    if n_cycles is None:
    ...
    sig_fft = np.fft.fft(sig_dc)
    fft_freqs = np.fft.fftfreq(n_samples, d=1.0 / sf)

    baseline = np.empty(len(freqs))
    for i, freq in enumerate(freqs):
        sigma_f = freq / n_cycles_arr[i]
        wavelet_fft = np.exp(-0.5 * ((fft_freqs - freq) / sigma_f) ** 2)
        if L2normalize:
            wavelet_fft /= np.sqrt(np.sum(wavelet_fft ** 2))
        analytic = np.fft.ifft(sig_fft * wavelet_fft)
        power = np.abs(analytic) ** 2
        baseline[i] = np.median(power)
    return baseline


def fit_1f_whitening_curve(freqs: np.ndarray, baseline_power: np.ndarray) -> np.ndarray:
    """
    Fit een power-law (1/f) trend door de whole-night mediane power per
    frequentie heen, via lineaire regressie in log-log-ruimte:
        log(power) ~ intercept + slope * log(freq)
    en geeft de GLADDE, gefitte trend terug (dus niet de ruwe empirische
    baseline zelf) -- dit is de correctiecurve waarmee de ridge-power
    gewhitened wordt.

    Waarom de gefitte trend i.p.v. de ruwe baseline_power: baseline_power
    bevat naast de systematische 1/f-afname ook de echte, nacht-specifieke
    pieken/dalen in het spectrum (bv. iemand met van nature meer
    sigma-activiteit). Delen door de ruwe baseline zou die informatie
    wegnormaliseren en de ridge relatief maken t.o.v. "normaal voor deze
    persoon/nacht" (dat was de vorige aanpak). Door alleen de gladde
    1/f-trend te fitten en weg te delen, corrigeer je uitsluitend het
    oneerlijke voordeel dat lage frequenties anders altijd hebben (puur
    door de fysiologische 1/f-vorm van EEG-power), en blijft de rest van
    de absolute spectrale vorm -- inclusief echte pieken -- intact. De
    ridge kiest dus weer op basis van (gecorrigeerde) ABSOLUTE power, niet
    relatief t.o.v. de eigen nacht.
    """
    eps = np.finfo(np.float64).eps
    log_f = np.log(freqs)
    log_p = np.log(baseline_power + eps)
    slope, intercept = np.polyfit(log_f, log_p, deg=1)
    fitted_log_p = intercept + slope * log_f
    return np.exp(fitted_log_p)
...
def compute_ridge_features(sig: np.ndarray, sf: float, start_i: int, end_i: int,
                            freq_whitening_curve: np.ndarray | None = None) -> dict:
    """
    ...
    freq_whitening_curve: gladde 1/f-correctiecurve per frequentie (van
    fit_1f_whitening_curve), zelfde freqs-as als hier. Als gegeven, wordt
    de event-power PER FREQUENTIE eerst gedeeld door deze curve vóór de
    argmax -- dit corrigeert alleen de systematische 1/f-scheefheid (lage
    frequenties hebben van nature meer ruwe EEG-power), niet voor de
    werkelijke nacht-specifieke spectrale vorm. De ridge blijft daardoor
    een keuze op basis van (gecorrigeerde) ABSOLUTE power, i.p.v. relatief
    t.o.v. de eigen nacht. Zonder freq_whitening_curve (None) valt dit
    terug op ongecorrigeerde power.
    """
    empty = {
    ...
    power_padded = compute_morlet_tf(padded_segment, sf, freqs, n_cycles=RIDGE_N_CYCLES, L2normalize=True)
    ...
    if freq_whitening_curve is not None:
        # Corrigeer per frequentie alleen voor de gefitte 1/f-trend -- dit is
        # de fix voor de 1/f-bias die eerder (ook met L2normalize=True) nog
        # steeds de laagste frequentie liet winnen, ZONDER de ridge relatief
        # t.o.v. de eigen nacht te maken (zie fit_1f_whitening_curve). eps
        # voorkomt delen door (bijna) 0.
        eps = np.finfo(np.float64).eps
        power = power / (freq_whitening_curve[:, None] + eps)
...
def extract_event_features(signals: dict, start_sec: float, end_sec: float,
                            ...
                            oxy_night_std: float | None,
                            ridge_freq_whitening_curve: np.ndarray | None = None) -> dict:
    """
    ...
    ridge_freq_whitening_curve: gefitte 1/f-correctiecurve (van
    fit_1f_whitening_curve, gebaseerd op de whole-night ridge-baseline,
    één keer per nacht berekend), wordt doorgegeven aan
    compute_ridge_features om alleen de systematische 1/f-scheefheid uit
    de event-power te corrigeren vóór de argmax -- de ridge blijft zo een
    keuze op basis van (gecorrigeerde) absolute power, i.p.v. relatief
    t.o.v. de nacht (in tegenstelling tot de band-ratio's, die WEL relatief
    t.o.v. de nacht zijn).
    ...
    if "EEG L" in signals:
        ...
        ridge_feats = compute_ridge_features(sig, sf, start_i, end_i,
                                              freq_whitening_curve=ridge_freq_whitening_curve)
...
def process_night(night_dir: Path, ids: dict, inspect: bool = False,
    ...
    # Whole-night ridge-power-baseline (per frequentie) + gefitte 1/f-whitening-
    # curve daaruit: zelfde eenmalig-per-nacht aanpak als de band-envelopes
    # hierboven, nodig om de per-event ridge tegen de 1/f-bias te corrigeren
    # zonder 'm relatief t.o.v. de eigen nacht te maken (zie
    # compute_night_ridge_baseline / fit_1f_whitening_curve).
    ridge_freqs = np.linspace(RIDGE_FREQ_MIN, RIDGE_FREQ_MAX, RIDGE_N_FREQS)
    if "EEG L" in signals:
        ridge_freq_baseline = compute_night_ridge_baseline(
            signals["EEG L"], TARGET_SFREQ, ridge_freqs, n_cycles=RIDGE_N_CYCLES, L2normalize=True
        )
        ridge_freq_whitening_curve = fit_1f_whitening_curve(ridge_freqs, ridge_freq_baseline)
    else:
        ridge_freq_whitening_curve = None
    ...
    for i, ev in events.iterrows():
        feats = extract_event_features(signals, ev["start_sec"], ev["end_sec"],
                                        ...
                                        ridge_freq_whitening_curve=ridge_freq_whitening_curve)
        feats.update({
